...
> **Finding (Osiris `LINK.PRG` bug, root-caused).** c68k's objects — including the full single-TU `libc.o` — link cleanly, but `LINK.PRG` address-exceptions on the 28-member `libieee754d.a`. Traced under **sid68k** (`!ex catch addr` on `sim68k --gdb`, unstripped LINK re-link for symbols): the fault is `move.l (a5),d1` in `lk_sym_add_object` (`lk_sym.a68`) with `A5` **odd** (`0x0EFABB`). `A5` is an archive member's ELF `.symtab` pointer, walked in place as `Elf32_Sym[16]`; member `dpmath.o` has its `.symtab` at an **odd file offset (`0x18d3`)`, and since the member base is even (`ar` 2-byte aligned) the in-place pointer is odd → the 68000 can't `move.l` from it. (1-member archives escape; it's neither an `ar`-format nor memory issue — 16 MB faults identically.) **Fix (Osiris/worm68k, not c68k):** make LINK read `Elf32_Sym` fields byte-wise/unaligned, or have `asm68K` emit `.symtab` at an even/4-aligned file offset. Repro tool: [`tools/osiris/debug-link-fault.ps1`](../tools/osiris/debug-link-fault.ps1). Hosted **soft-float** programs use the cross linker (`m68k-elf-ld`, lockstep 8/8) meanwhile.

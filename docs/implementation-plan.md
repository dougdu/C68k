...
> **Finding (Osiris `LINK.PRG` bug, not memory).** c68k's objects — including the full single-TU `libc.o` — link cleanly, but `LINK.PRG` address-exceptions on the 28-member/125 KB `libieee754d.a` float archive. It is **not** a memory limit: under `--cpu 68000 --mem MAX` (16 MB) it faults at the *identical* fixed location (PC `a8212`, odd target `efabb`) as under the default 68008/1 MB model — an odd-pointer dereference from a mis-parse of the large multi-member archive (rebuilding it deterministically doesn't help; a 1-member archive links fine). So hosted **soft-float** programs still use the cross linker (`m68k-elf-ld`, lockstep 8/8) until this `LINK.PRG` `ar`-reader bug is fixed — an Osiris-toolchain follow-on, not a c68k issue.

...
- [x] Native recipes: Osiris `CC→LINK→.PRG`; CP/M `CC→LINK→mkdri→.68K`. *(Osiris `CC→LINK→.PRG` and `CC→LIB→LINK→.PRG` done; CP/M `LINK.68K`/`LIB.68K` link/archive **byte-identically** to `ld`/`ar`. **CP/M output-layout mode done:** under `/DCPM`, `lk_layout`/`lk_reloc`/`lk_out` emit an absolute `ET_EXEC` at `0x500` (no PIE — `.text`@`0x500`, two `PT_LOAD`, `R_68K_32` resolved to absolute addresses, no `.hash`/`.dynsym`/`.dynamic`/`.got`/`.rela.dyn`), so `mkdri` yields a **runnable** CP/M `.68K` — byte-identical to `ld -T cpm68k.ld`'s `.68K`. Test [`worm68k\cpm68k\link\test-cpmlink.ps1`] links `HELLO.O` with `LINK.68K` on CP/M under `sim68k`, `mkdri`'s LINK's output, and runs it (prints its banner). The plain Osiris (`no /DCPM`) build is re-verified **byte-identical** — every LINK/objlib `.o` unchanged.)*
- [x] A multi-object + archive program links natively on both OSes and runs under `sim68k`. *(Osiris — multi-TU + `LIB` archive both run (`MULTI TU OK`). CP/M — `LINK.68K` links a multi-object program and consumes a `LIB.68K` archive, output byte-identical to `ld`/`ar`; the CP/M absolute-`0x500` output now `mkdri`'s to a **runnable** `.68K` that executes under `sim68k` (see the output-layout mode above).)*

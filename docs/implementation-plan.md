...
- [x] Native recipes: Osiris `CC→LINK→.PRG`; CP/M `CC→LINK→mkdri→.68K`. *(Osiris `CC→LINK→.PRG` and `CC→LIB→LINK→.PRG` done; CP/M `LINK.68K`/`LIB.68K` link/archive **byte-identically** to `ld`/`ar`. Remaining follow-on: `LINK.68K`'s output layout is still the hardwired `osiris-prg.ld` static-PIE `.PRG` — a CP/M absolute-`0x500` output-layout mode in `lk_layout`/`lk_out` (so `mkdri` yields a **runnable** CP/M `.68K`) is the last step of the CP/M recipe.)*
- [x] A multi-object + archive program links natively on both OSes and runs under `sim68k`. *(Osiris — multi-TU + `LIB` archive both run (`MULTI TU OK`). CP/M — `LINK.68K` links a multi-object program and consumes a `LIB.68K` archive, output byte-identical to `ld`/`ar`; running the CP/M output waits on the output-layout follow-on above.)*

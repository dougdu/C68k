...
| **OP7 v2** | F·G | [Value materialization & full allocation](#op7-v2--value-materialization--full-allocation) | O3 | ◐ | 3 / 8 | CSE-mat / LICM / IV-SR + real spill / live-range splitting + `break`/`continue` |
| | | **Total** | | **6 / 8** | **30 / 33 core · 3 / 8 v2** | |
...
**Status: ◐ in progress (scoped 2026-08-03; V1+V2 landed 2026-08-03, V3 landed 2026-08-04).** OP7 v1's interference allocator handles source-variable
...
- [x] **V2 — CFG dataflow liveness. Done 2026-08-03.** Replaced the linear-scan interval approximation
      with backward live-in/live-out dataflow over the CFG ([ir68k.c](../src/ir68k.c) `ra_liveness`): per-block
      upward-exposed uses / must-kill defs are read from the IR expressions, `out[b] = ∪ in[succ]` and
      `in[b] = use[b] ∪ (out[b] − def[b])` iterate to a fixpoint, and interference is built by walking each
      block backward (every def clobbers whatever is live, so dead defs are handled; everything live at entry
      interferes pairwise so promoted params never share). A store on only one `?:`/`&&`/`||` arm is a
      **may-def** (clobbers but does not kill). Allocation moved **after** the CFG + `-O3` opts are built
      (`ir_plan_regs` split into `ir_prep_regs` before lowering + `ir_color_regs` after), so it runs on the
      optimized graph. Interference is now exact (no false conflicts from interval holes) and correct across
      arbitrary edges — the basis for V3. Same coloring / gate as v1, so structured code is unchanged (opt-check
      `regshare` still `movem d2-d4`). **Validated:** opt-check 20/20, opt-measure identical (-O2 518/1022,
      -O3 508/954), **self-host stage3 14/14 byte-identical**, **lockstep 27/27 on both OSes** (REGALLOC 14/14).
- [x] **V3 — Structured `break`/`continue`. Done 2026-08-04.** The -O3 allocator gate
      ([ir68k.c](../src/ir68k.c) `ra_has_user_jump`) now admits functions whose only unstructured edges are
      chibicc's `break`/`continue`-lowered gotos (`ND_GOTO` with no source label), which V2's CFG dataflow
      liveness already resolves exactly. Only a **user** `goto` (`ND_GOTO` with a source `label`) or a user
      `ND_LABEL` — possibly irreducible — still falls back to the OP5 per-variable pass (switch/case make the
      function IR-ineligible in `stmt_ok`, so no synthetic labels reach the IR). The CFG wires break/continue
      edges through `block_of_label` like any jump, and backward liveness is a correct over-approximation on
      any CFG (unreachable/extra blocks only add conservative interferences), so sharing across these edges is
      safe. Example: two disjoint break/continue loops go from `movem d2-d5` (OP5) to `movem d2-d4` (the two
      indices share a register). **Validated:** opt-check 20/20, opt-measure -O3 508/954 (no regression),
      **self-host stage3 14/14 byte-identical** (CC.PRG rebuilt at -O3 with V3, so the on-target compiler runs
      V3-allocated break/continue code), **lockstep 27/27 on both OSes** (REGALLOC 16/16, incl. a new
      `breakshare` sharing case). **Basis for V4.**
...
| 2026-08 | Draft 0.1 | **OP7 v2 · V3 done (`-O3`).** The interference allocator gate ([`ir68k.c`](../src/ir68k.c) `ra_has_user_jump`) now admits functions whose only unstructured edges are `break`/`continue` (`ND_GOTO` with no source label) — V2's CFG liveness resolves them exactly, so their disjoint ranges can share registers (e.g. two break/continue loops go `movem d2-d5` → `movem d2-d4`). A user `goto`/`ND_LABEL` (possibly irreducible) still falls back to OP5. Byte-neutral for -O0/-O1/-O2 and -O3 goto/goto-free code; opt-check 20/20, opt-measure -O3 508/954 unchanged; self-host stage3 14/14 byte-identical (CC.PRG rebuilt at -O3 with V3); lockstep 27/27 × 2 OSes (REGALLOC 16/16, new `breakshare` case). Basis for V4. |
| 2026-08 | Draft 0.1 | **OP7 v2 · V2 done (`-O3`).** CFG dataflow liveness ([`ir68k.c`](../src/ir68k.c) `ra_liveness`) replaces the v1 linear-scan intervals: backward live-in/out fixpoint over the basic-block CFG, exact def-clobbers-live interference (dead defs + may-defs on `?:`/`&&`/`||` arms handled; entry-live params interfere pairwise). Allocation moved after the CFG + `-O3` opts (`ir_plan_regs` split into `ir_prep_regs`/`ir_color_regs`). Structured code unchanged (opt-check 20/20, opt-measure identical); self-host stage3 14/14 byte-identical; lockstep 27/27 × 2 OSes (REGALLOC 14/14). Basis for V3 (break/continue). |

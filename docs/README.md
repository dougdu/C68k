...
| 4 | [sdk.md](sdk.md) | **SDK quickstart** for third-party programs: how to compile with `c68k`, the driver options and predefined macros (`__c68k__`/`__osiris__`/`__CPM68K__`), the per-OS link recipes (`.PRG` / `.68K`), and a worked one-source, two-target `hello` example. |

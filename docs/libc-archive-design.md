...
| 6 | Native archives + docs + final size win | ✅ | native Osiris `LINK.PRG` member-selects the c68k archives (merged for its single-archive linker) — hello/printftest link+run; `LIB.PRG` builds an archive `LINK` consumes; docs + size win recorded. Deferred: CP/M native `LINK`/`LIB`, native-`LINK` multi-archive, native-`LINK` bus-error linking libheap members |
...
- **Deferred follow-ons** (out of the chosen scope, filed for later): (1) **CP/M native `LINK`/`LIB`** (DRI `mkdri`; the 1 MB-wall + DRI-port effort tracked since P9). (2) Native `LINK.PRG` **multi-archive** support — it currently searches one archive per link; the host-side merge is the workaround. (3) **`LINK.PRG` faults while linking the libheap objects** — pulling any libheap member makes the native `LINK.PRG` **bus-error during member selection** (it never emits the `.PRG`; this is a linker crash, *not* a runtime or `.bss` problem). Caught under `sid68k` (`!ex catch bus`): the fault is `tst.b (a0)+` (IR `4A18`) in objlib's `lo_strlen`, called from `lo_find_container`, on a container/object whose **name pointer is `0xFFFFFFFF`** (`A4=FFFFFFFF`) — i.e. an object registered while pulling a libheap member is given a garbage name, and the linker's name scan then reads `0xFFFFFFFF`. Same class as the P9 float-archive `LINK` crash (the linker walking a member structure through a bad pointer), a different site. `hello`/`printftest` (libc/libm) link fine, and the cross `ld` links libheap correctly (lockstep **10/10**). So native **heap** programs await the `LINK.PRG` fix; the cross toolchain already has full libheap.

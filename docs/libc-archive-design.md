...
| 6 | Native archives + docs + final size win | ✅ | native Osiris `LINK.PRG` member-selects the c68k archives (merged for its single-archive linker) — hello/printftest/memtest link+run stripped (memtest `MEM PASS 15/15`); `LIB.PRG` builds an archive `LINK` consumes; docs + size win recorded. Deferred: CP/M native `LINK`/`LIB`, native-`LINK` multi-archive, `LINK` unstripped symbol capacity |
...
- [x] **Native heap programs link and run** — the native link now **strips** (`-s`), matching the cross `ld` default. This is required, not cosmetic: unstripped, `LINK.PRG` builds a full `.symtab`, but its fixed buffers (`LL_MAXSYM` = 1024 symbols; `LM_SLACK` = 64 KB image tail) overflow once a link pulls enough objects — the libheap members push the symbol count to ~6 k, which corrupts the symtab and bus-errors the linker (root-caused under `sid68k` `!ex catch bus`: `tst.b (a0)+` in objlib `lo_strlen` ← `lo_find_container` reading a `0xFFFFFFFF` symbol-name pointer — an `lo_gsc[i]`/`.symtab` overrun in `lo_build_strtab`). Stripping skips that whole path. Verified: **memtest** (full allocator stress) links **and runs** `MEM PASS 15/15` on-target.
- **Deferred follow-ons**: (1) **CP/M native `LINK`/`LIB`** (DRI `mkdri`; the 1 MB-wall + DRI-port effort tracked since P9). (2) Native `LINK.PRG` **multi-archive** support — it searches one archive per link; the host-side merge is the workaround. (3) `LINK.PRG` **unstripped symbol capacity** (`LL_MAXSYM`/`LM_SLACK` too small for large links) — only needed for `-g` native links; the cross `ld` handles unstripped.

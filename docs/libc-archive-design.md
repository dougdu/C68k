...
| 6 | Native archives + docs + final size win | ✅ | native Osiris `LINK.PRG` member-selects the c68k archives (now **multi-archive** — `libc.a`+`libm.a`+`libheap.a` passed separately, no host merge) — hello/printftest/memtest link+run (memtest `MEM PASS 15/15`); symtab scales, all internal tables bounds-checked and grow on demand (unstripped links work too); `LIB.PRG` builds an archive `LINK` consumes; docs + size win recorded. Deferred: CP/M native `LINK`/`LIB` on-target smoke (shares source, inherits the fixes) |
...
- [x] **Native Osiris `LINK.PRG` member-selects the c68k archives** ([run-native-link.ps1](../tools/osiris/run-native-link.ps1)) — the whole-archive `ld -r --whole-archive` `LIBC.O` blob is gone; the native linker now dead-strips per object exactly like the cross `ld`. The native `LINK.PRG` now handles **multiple archives per link** (`lm_arctxs[]` plus a fixpoint sweep that repeats until no new member is pulled, so a member in one archive can satisfy an undefined symbol in another), so `libc.a` + `libm.a` + `libheap.a` are passed as **three separate archives** — no host-side `ar -M` merge. Verified on the 16 MB Osiris model (`--cpu 68000 --mem MAX`): **hello** (libc `puts`→`fputs` member selection), **printftest** (cross-archive: libc `printf` pulling libm float-format members; `%f` + 64-bit), and **memtest** (libheap `malloc`/`free`/`realloc` stress) link and **run correctly** (`MEM PASS 15/15` on-target).
...
- [x] **Native heap programs link and run** — verified **memtest** (full allocator stress) links **and runs** `MEM PASS 15/15` on-target. The default remains a **stripped** native link (`-s`, cross-`ld` parity, and fastest). Unstripped native links also work now: `LINK.PRG` previously overran fixed symtab buffers (`LL_MAXSYM` = 1024; `LM_SLACK` = 64 KB tail) at ~6 k symbols and bus-errored in `lo_build_strtab` (root-caused under `sid68k` `!ex catch bus`: an `lo_gsc[i]`/`.symtab` overrun → `0xFFFFFFFF` name pointer → `tst.b (a0)+` in objlib `lo_strlen`); the symtab/strtab image is now **sized to the actual symbol count** and `lo_emitsym` is **bounds-checked**, so it no longer overruns. (Large unstripped links are correct but slow — `lo_build_strtab` is an O(n²) suffix-merge; small unstripped links, e.g. `hello -NoStrip`, are instant.)
- [x] **`LINK.PRG` scale/robustness hardening** — (1) **multi-archive** per link (above); (2) **every internal table bounds-checks** with a graceful linker error instead of silent corruption; (3) **every internal table now grows on demand** — the symbol-resolver arena chains fresh `cl_alloc` pools (bucket heads in pool 0 never move), the output symtab/strtab scale to the symbol count, and the object tables (`lm_objs`/`objptrs`/`objnames`/`nameptrs`), the pulled-member list, and the archive-context table all double via a shared alloc-bigger+copy helper (`lk_grow`); OOM is handled gracefully at every site. Growing `lm_objs` is safe because a symbol carries an object **index**, not a pointer, so the copied `objptrs`/`nameptrs` mirrors are simply recomputed from the new bases. The former `LL_MAXOBJ`/`LK_MAXPULL`/`LK_MAXARC` ceilings are now just **initial capacities** (verified by forcing them to 1–4 and re-linking `memtest`/`printftest` through many doublings). The linker source is **shared with CP/M `LINK.68K`** (`ifdef CPM`) and all six modules assemble clean with `CPM` defined, so `LINK.68K` inherits every fix.
- **Deferred follow-on**: **CP/M native `LINK`/`LIB` on-target smoke** — no CP/M linker build/test harness exists yet (the 1 MB-TPA + DRI `mkdri` bring-up is tracked since P9); the shared-source fixes are already in place for when it lands.
...
| 2026-07-22 | Phase 6 | **Native Osiris `LINK.PRG` consumes the c68k archives via member selection.** [run-native-link.ps1](../tools/osiris/run-native-link.ps1) dropped the whole-archive `ld -r` `LIBC.O` blob; because the native `LINK.PRG` searches a **single** archive per link, `libc.a`+`libm.a`+`libheap.a` are merged (`ar -M addlib` → `LIBC68K.A`, 146 members) and member-selected on-target. Verified on the 16 MB model: **hello** (libc) and **printftest** (cross-archive libc→libm `%f`) link+run; **`LIB.PRG` builds an archive `LINK` consumes** (`MULTI TU OK`). **Final dead-strip win** recorded (bare 88,268→2,756 = −97%; hello −88%); `CC.PRG` 467,372, lockstep **10/10**. Docs updated ([sdk.md](sdk.md)/[reference-manual.md](reference-manual.md)/[libc-and-toolchain.md](libc-and-toolchain.md)). **Native heap programs link+run once the native link strips** (`-s`, parity with the cross ld): unstripped, `LINK.PRG`'s fixed symtab buffers (`LL_MAXSYM`=1024 / `LM_SLACK`=64 KB) overflow at ~6 k symbols and bus-error in `lo_build_strtab` (`lo_gsc`/`.symtab` overrun; root-caused via `sid68k !ex catch bus`); `-s` skips it → **memtest `MEM PASS 15/15`** on-target. Subsequently hardened: `LINK.PRG` now handles **multiple archives** (no host merge), **scales its symtab + bounds-checks all internal tables**, and **grows every internal table on demand** — the resolver arena chains pools, and the object/pulled-member/archive tables double via a shared `cl_alloc`-based `lk_grow` helper — so unstripped native links work too and no fixed ceiling can be hit; CP/M `LINK.68K` inherits the fixes via the shared source (assembles clean `/DCPM`). |

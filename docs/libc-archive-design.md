...
| 6 | Native archives + docs + final size win | ✅ | native Osiris `LINK.PRG` member-selects the c68k archives (now **multi-archive** — `libc.a`+`libm.a`+`libheap.a` passed separately, no host merge) — hello/printftest/memtest link+run (memtest `MEM PASS 15/15`); symtab scales, all tables bounds-checked, resolver arena grows (unstripped links work too); `LIB.PRG` builds an archive `LINK` consumes; docs + size win recorded. Deferred: CP/M native `LINK`/`LIB` on-target smoke (shares source, inherits the fixes) |
...
- [x] **`LINK.PRG` scale/robustness hardening** — (1) **multi-archive** per link (above); (2) **every internal table bounds-checks** with a graceful linker error instead of silent corruption; (3) the **symbol-resolver arena grows on demand** (chained `cl_alloc` pools; bucket heads in pool 0 never move, so no pointer is invalidated) and the output symtab/strtab scale to the symbol count. The linker source is **shared with CP/M `LINK.68K`** (`ifdef CPM`) and all six modules assemble clean with `CPM` defined, so `LINK.68K` inherits every fix.
- **Deferred follow-ons**: (1) **CP/M native `LINK`/`LIB` on-target smoke** — no CP/M linker build/test harness exists yet (the 1 MB-TPA + DRI `mkdri` bring-up is tracked since P9); the shared-source fixes are already in place for when it lands. (2) A few small member-selection tables (`lm_objs` = 80, pulled = 64, arctxs = 16) keep a fixed — but generous — limit with graceful bounds; converting them to dynamic growth is mechanical.

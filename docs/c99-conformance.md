...
| `<stdio.h>` | Input/output | ✅ | ⚠️ | `libc/include/stdio.h`, `libc/core/*.c` | Full narrow + wide `printf`/`scanf`, `fopen` text/binary + `+` update modes (orientation‑tracked), `fread`/`fwrite`, `ungetc`, buffer‑aware `ftell`, `fgetpos`/`fsetpos`, `tmpnam`/`tmpfile`, `remove`/`rename`, `setvbuf` (adopts a caller buffer), and `freopen` (path **and** NULL‑path mode change). Only `gets` is intentionally omitted (unsafe; removed in C11). On CP/M‑68K `fseek(…,SEEK_END)` is record‑granular and in‑place random *modify* is best‑effort (128‑byte record model). See §stdio. |
...
| `freopen` | Reassign stream | ✅ | ✅ | libc / `freopen.c` | Flush+close then reopen, reusing the FILE (so `freopen("f","w",stdout)` redirects). A NULL path changes the open stream's mode in place (text/binary/orientation), keeping the fd. |
| `setbuf` / `setvbuf` | Buffering control | ✅ | ✅ | libc / `setvbuf.c` | Adopts a caller‑supplied buffer of the given size (`_IOFBF`/`_IOLBF`); a NULL buffer keeps the built‑in `BUFSIZ` buffer; `_IONBF` is unbuffered. Must be called before any I/O (C99). |
...
wide engine (dead‑stripping).

Wide **formatted input** completes the set (`libc/core/vswscanf.c`): `fwscanf`,
`wscanf`, `swscanf`, and the `v*` variants.  `_vwscan` mirrors the narrow
`_vscan` over a `wchar_t` source (a wide‑oriented `FILE` via `fgetwc`/`ungetwc`,
or a `wchar_t` string); numeric tokens are narrowed to ASCII and handed to the
same `strtoll`/`strtoull`/`strtod` primitives, while `%c`/`%s`/`%[` store a
multibyte (UTF‑8) char array by default and a `wchar_t` array under the `l`
modifier.  Also in its own object for dead‑stripping.
...
7. **`<math.h>`** — Phase 2a ✅ DONE (2026‑07‑22): added `HUGE_VAL`/`INFINITY`/
   ...
   both OSes.
   - Phase 2b ✅ DONE (2026‑07‑22): `sinh`/`cosh`/`tanh`, `asinh`/`acosh`/
